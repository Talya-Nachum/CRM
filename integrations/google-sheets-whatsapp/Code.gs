 ...
 * דפדפן, בלי URL, בלי פריסה בכלל. מריצים ישירות מהעורך. שגיאה כאן
 * אומרת שיש בעיה אמיתית בקוד עצמו; הצלחה כאן אומרת בוודאות שהקוד
 * תקין וכל הבעיה היא בפריסה/בכתובת שנפתחת בדפדפן - לא בקובץ.
 * להזין למטה קוד אמיתי של נציגה (מהטאב "נציגות", עמודת "קוד אישי")
 * ולהריץ.
 */
function debugRepTemplate() {
  const testRepKey = 'הדביקי כאן קוד נציגה אמיתי';
  try {
