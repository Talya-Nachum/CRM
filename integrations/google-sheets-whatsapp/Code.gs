...
// הסטטוסים שנציגה יכולה לבחור כשהיא מסמנת "סיימתי לטפל" - לחיצה אחת
// בלי גלילה. "אחר..." פותח את הרשימה המלאה.
const REP_QUICK_STATUSES_ = ['תואמה פגישה', 'בתהליך', 'בתהליך עתידי', 'לא מעוניין', 'סגור פניה'];
// הסטטוס שמפעיל קונפטי אצל הנציגה (השאר מקבלים אנימציית לבבות).
const REP_CELEBRATE_STATUS_ = 'תואמה פגישה';
// הסטטוס היחיד שמציג לנציגה את ההודעה "תודה שניסית" **וחוזר לטליה
// לבדיקה נוספת**. כל סטטוס אחר (כולל "סגור פניה"/"בתהליך עתידי") נסגר
// בשקט בלי הודעה - לבקשת הלקוחה: רק "לא מעוניין" באמת דורש שהיא תסתכל
// עליו שוב.
const REP_SORRY_STATUS_ = 'לא מעוניין';
...
const DEFAULT_STATUS_LIST_ = [
  'שגוי', 'אין פרטי התקשרות', 'סגור פניה', 'לא מעוניין', 'לא לפנות', 'כפול', 'כללי', 'חברה נסגרה',
...
function orderedPearlSheets_(ss, incomingPhone, eventPearlId) {
  ...
  // ב-startCallsInSheet_), מהחדש לישן. אם גם זה לא ידוע (הליד נטען ישירות
  // לפרלה בלי לעבור דרך המערכת שלנו) - שום טאב, בכוונה: אין דרך לדעת
  // בוודאות לאיזה קמפיין השיחה שייכת, ועדיף "לא התעדכן" (סימן שהשיחה
  // לא יצאה מכאן) על פני ניחוש שעלול לעדכן קמפיין לא-קשור.
  return trackedCampaignSheets_(ss, incomingPhone);
}
...
function handleNlpearlWebhook_(ss, payload) {
  ...
  // הסטטוס: אם פרלה שלחה תגית לשיחה הזו - היא **המקור היחיד**, מילה
  // במילה, בלי שום ניחוש/סיווג שלנו (בקשה מפורשת של הלקוחה - "בדיוק
  // כמו שפרלה כותבת"). רושמים אותה ב"רשימת סטטוסים" אם היא עוד לא שם
  // (בדיוק כמו addStatusIfMissing_ לכפתורי נציגה) כדי שיהיה לה צבע
  // עקבי ושהיא לא "תיעלם" מהתצוגה כי היא לא באוצר המילים הסגור.
  // רק כשאין תגית בכלל (לא כל שיחה מקבלת אחת) נופלים חזרה לסיווג
  // מהתמלול/הסיכום - כללים דטרמיניסטיים, ואז AI (matchCallStatus_,
  // קורא ל-Gemini) אם הכללים לא הכריעו.
  //
  // סיווג ב-AI קורא לרשת - יכול לקחת כמה שניות טובות. באג אמיתי שנתפס
  // בפועל: זה קרה **בתוך** הנעילה, כך שכל קריאת Webhook אחרת (גם
  // לגמרי לא קשורה - וואטסאפ, קמפיין אחר) הייתה נתקעת מאחוריה. כשכמה
  // שיחות פרלה הגיעו כמעט יחד (פרלה שולחת כמה אירועים ברצף על אותה
  // שיחה), התור הזה גדל עד שקריאה מסוימת חיכתה יותר מ-10 שניות
  // ל-waitLock, קיבלה שגיאת timeout, ו-doPost בלע אותה בשקט (רושם
  // ERROR ב-WebhookLog ומחזיר "ok" - כדי שפרלה לא תנסה שוב ותכפיל
  // נתונים) - כך שהתמלול/הסיכום שכן הגיעו פשוט לא נכתבו, בלי שום עדות
  // ל"תקלה" מלבד שורת ERROR ב-WebhookLog שלא כוללת את הטלפון. לכן
  // קוראים ל-Gemini **לפני** הנעילה - הנעילה עכשיו מגנה רק על
  // הקריאה/כתיבה בגיליון עצמו, שהיא מהירה.
  let statusValueFromAi_;
  if (tagText) {
    // רושמים לפני שקוראים את הרשימה - אחרת isAllowedStatusValue_
    // למטה יבדוק מול רשימה ישנה שעדיין לא כוללת את התגית שהרגע נוספה.
    addStatusIfMissing_(tagText);
    statusValueFromAi_ = tagText;
  } else {
    statusValueFromAi_ = matchCallStatus_(callTextForStatus_(transcript, summary, tagText), getStatusList_());
  }
    ...
    for (const sheet of sheets) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (sheetPhone && sheetPhone === incomingPhone) {
          ...
          const statusCol = findColumnNormalized_(headers, CALL_STATUS_HEADER);
          // הסטטוס: תגית של פרלה (אם יש) - מילה במילה, בלי סיווג שלנו
          // (בקשה מפורשת של הלקוחה). רק בלי תגית נופלים לתמלול/סיכום +
          // AI. חושב למעלה, לפני הנעילה - ר' הערה שם.
          const statusValue = statusValueFromAi_;
          // אחידות מוחלטת: כותבים ל"סטטוס שיחה" **רק** ערך מאוצר המילים
          // הסגור (התגית כבר נרשמה שם למעלה אם היא חדשה). אם העיגול
          // ב-AI נכשל והוחזר הנרטיב הגולמי - לא כותבים כלום (הסטטוס
          // הקודם נשאר), והטקסט המלא ממילא כבר נשמר ב"סיכום שיחה" למעלה.
...
/** עטיפה להרצה ישירה - עדנה/ניר בטאב "טריפל סי", לא מוצגים בדשבורד. */
function debugEdna() {
  debugContactByPhone('0504441276');
}
 ...
 * שגיאה כאן אומרת שיש בעיה אמיתית בקוד עצמו; הצלחה כאן אומרת בוודאות
 * שהקוד תקין וכל הבעיה היא בפריסה/בכתובת שנפתחת בדפדפן - לא בקובץ.
 */
function debugRepTemplate() {
  const reps = activeReps_();
  if (!reps.length) {
    Logger.log('❌ אין אף נציגה בטאב "נציגות" - צריך להריץ קודם setupRepsSheet().');
    return;
  }
  try {
    const t = HtmlService.createTemplateFromFile('RepDashboard');
    t.baseUrl = ScriptApp.getService().getUrl();
    t.repKey = reps[0].key;
    const html = t.evaluate().getContent();
    Logger.log('✅ הצליח בלי שום שגיאה. אורך ה-HTML שנוצר: ' + html.length +
      ' תווים. המסקנה: הקוד עצמו תקין - הבעיה בהכרח בפריסה/בכתובת שנפתחת בדפדפן.');
  } catch (err) {
    Logger.log('❌ נכשל כאן גם - זו אכן בעיה אמיתית בקוד: ' + err.message + '\n' + err.stack);
  }
}
...
function assignLead(sheetName, phone, repName, noteToRep) {
  ...
  // נאמר, וגם כפתור ה-AI לא יוכל לסכם כלום. התמלול המלא נכנס גם הוא -
  // בלעדיו הנציגה (וגם ה-AI שלה) רואה רק סיכום של שתי שורות במקום מה
  // שבאמת נאמר בשיחה.
  //
  // **לא** כולל את USER_NOTES_HEADER ("הערות משתמש") בכוונה - זו העמודה
  // שבה writeBackRepStatus_ כותבת "סיימה לטפל · סטטוס: ... " בכל פעם
  // שנציגה מסיימת טיפול. באג אמיתי שנתפס בפועל: אם ליד עבר בעבר לנציגה
  // אחת שסיימה לטפל בו (ואז חזר למיטוב, למשל אחרי 9 שעות), וטליה העבירה
  // אותו הלאה לנציגה אחרת - ה"היסטוריה" של הנציגה **הקודמת** נכנסה
  // לתיעוד שהנציגה **החדשה** רואה, בלי שום קשר אליה. עכשיו הנציגה
  // החדשה רואה רק את תגובת הליד עצמו (וואטסאפ/פרלה) - לא הערות פנימיות
  // של נציגות קודמות.
  const snapshot = [
...
/**
 * כל ההקצאות (פתוחות + שטופלו) של נציגה לפי שם - למעקב של מיטוב מתוך
 * הדשבורד הראשי (לחיצה על נציגה בפאנל "לידים לפי נציגה"). אותה
 * לוגיקה בדיוק כמו getRepData, רק לפי שם ולא לפי הקוד הסודי - כי כאן
 * זו מיטוב (לא הנציגה) שמבקשת, ואין לה/אין צריך את הקוד.
 */
function getRepAssignmentsForAdmin(repName) {
  const rep = repByName_(repName);
  if (!rep) throw new Error('נציגה לא מוכרת: ' + repName);
  return getRepData(rep.key);
}
  ...
  try {
    ...
    for (let i = 0; i < data.values.length; i++) {
      ...
      return {
        ...
        sorry: normalizeLabel_(statusText) === normalizeLabel_(REP_SORRY_STATUS_)
      };
    }
    throw new Error('ההקצאה לא נמצאה');
  } finally {
...
const CALL_OUTCOME_RULES_ = [
  ...
  { status: 'סגור פניה', words: ['לא רלוונטי', 'מתחת ל-50', 'מתחת ל50', 'פחות מ-50', 'פחות מ50',
...
function revealSeamlessContacts(searchResultIds) {
  ...
  for (let attempt = 0; attempt < 9; attempt++) {
    ...
    if (attempt < 8) Utilities.sleep(2000);
  }
