/**
 * דף לידים חכם – אודיוקודס, שולחן עגול Meeting Insights (20.10.2026)
 * (הועתק ומותאם מהתבנית ב-templates/client-leads-page/ - ר' BUILD-GUIDE.md
 * שם לרשימת כל מה שנבדק/הותאם ללקוח הזה)
 *
 * גיליון Google Sheets נפרד + Apps Script Web App:
 *  - doPost: תשובות נכנסות מוואטסאפ (אינפוריו) - אותו חשבון אינפוריו כמו
 *    המערכת הראשית, ולכן חייב להיות מחובר ל-relay שלה (ר' README.md כאן).
 *    קליטת ליד אוטומטית מדף נחיתה לא בשימוש בשלב הזה - הלידים נטענים ידנית
 *    מאקסל - אבל נקודת הקצה נשארת זמינה לעתיד בלי שינוי קוד.
...
var CLIENT_NAME = 'אודיוקודס — שולחן עגול';
var CLIENT_TAGLINE = 'Meeting Insights במוסדות להשכלה גבוהה';
var TARGET_DATE = '2026-10-20T10:00:00+03:00'; // תחילת האירוע (יום ג׳ 20.10.2026, 10:00)
var PAYMENT_URL = ''; // לא רלוונטי לאירוע הזה (אין תשלום/כרטיס) - כפתור התשלום הוסר מהממשק בכוונה
// אם וכשתישלח תמונת ההזמנה המקורית (קישור Drive משותף), אפשר למלא כאן -
// ואז יופיע קישור "לצפייה בתמונת ההזמנה המקורית" בתוך מודאל "מידע על הכנס".
// עד אז המודאל מציג שחזור תוכני (כותרת/תאריך/כתובת/לוח זמנים) שכבר בנוי
// ישירות ב-Index.html, כי אין גישה לקובץ התמונה שנשלח בצ'אט (ר' BUILD-GUIDE.md,
// "מגבלות סביבת הפיתוח").
var INFO_URL = '';
...
// רשימת הסטטוסים הראשונית לתהליך הרישום לאירוע - טליה תוסיף/תסיר עוד ישירות
// בטאב "רשימת סטטוסים" בגיליון לפי הצורך (זו ההרשאה היחידה, ר' README.md).
// "נשלח וואטסאפ" חייב להישאר ברשימה - הקוד קובע אותו אוטומטית אחרי שליחה
// מוצלחת (ר' WHATSAPP_SENT_STATUS/sendWhatsApp למטה). "נרשם לבד"/"נרשם ע"י
// מיטוב" הם שני הסטטוסים שנספרים בבאנר "נרשמו עד כה" (ר' REGISTERED_STATUSES
// ב-JavaScript.html).
var DEFAULT_STATUSES = ['בתהליך', 'נרשם לבד', 'נרשם ע"י מיטוב', 'נשלח וואטסאפ', 'לא מעוניין'];

// הסטטוס שנקבע אוטומטית לכל ליד שנבחר בשליחת וואטסאפ מוצלחת (ר' sendWhatsApp)
var WHATSAPP_SENT_STATUS = 'נשלח וואטסאפ';

// מספר התבנית האמיתי שכבר קיים באינפוריו - טליה תוסיף עוד שורות בעצמה
// בטאב "תבניות" בגיליון בלי לגעת בקוד.
var DEFAULT_TEMPLATES = [
  ['הזמנה לשולחן עגול', '285658']
];
...
var LEAD_FIELDS = [
  ...
  { key: 'replyText', label: 'תשובת איש קשר' },
  { key: 'replyAt', label: 'תאריך תשובה' }
];

// מיפוי שמות שדות אפשריים שיגיעו מדף הנחיתה (בעברית/אנגלית) לשדות שלנו -
// לא בשימוש כרגע (הלידים נטענים ידנית מאקסל), נשאר מוכן לעתיד בלי שינוי קוד.
var FIELD_ALIASES = {
...
/**
 * נקודת קצה ל-Webhook. תומך ב-JSON וגם בטופס רגיל (form-urlencoded).
 * כרגע משמש בפועל רק לתשובות נכנסות מוואטסאפ (ר' handleInforuReply_ למטה) -
 * קליטת ליד חדש מדף נחיתה לא בשימוש כי הלידים נטענים ידנית מאקסל, אבל
 * נשאר מוכן בלי שינוי קוד אם וכשיהיה דף נחיתה.
 */
function doPost(e) {
  try {
    ensureSheets_();
...
function ensureSheetsLocked_() {
  ...
  // "רשימת סטטוסים" - טליה שולטת בה ישירות מהגיליון (להוסיף/להסיר שורות
  // בעמודה A). הדשבורד רק קורא ממנה, אין כפתור עריכה בממשק בכוונה - הגישה
  // לגיליון עצמו היא ההרשאה היחידה. חריג יחיד: WHATSAPP_SENT_STATUS - סטטוס
  // שהקוד עצמו קובע אוטומטית אחרי שליחה מוצלחת (ר' sendWhatsApp), אז הוא
  // חייב להיות ברשימה כדי שיוצג נכון בתפריטים, גם אם הטאב כבר היה קיים לפני
  // שהתכונה הזו נוספה.
...
/**
 * קוראת את כל הלידים, כולל שורות שנוספו ידנית ישירות בגיליון - זו הדרך
 * העיקרית שבה טליה מזינה לידים לאירוע הזה (הדבקה מאקסל), לא רק דרך כפתור
 * "הוספת ליד" או ה-webhook. שורה ידנית לרוב לא תמלא "מזהה"/"תאריך יצירה"/
 * "סטטוס" (עמודות שהדשבורד מנהל לבד) - הפונקציה משלימה אותן אוטומטית בפעם
 * הראשונה שהיא נקראת, כדי שעריכה/מחיקה/שליחת וואטסאפ מהדשבורד יעבדו גם על
 * שורה כזו מכאן ואילך.
 */
function getLeads() {
  ensureSheets_();
...
/**
 * קוראת את טווח הנתונים כולל שורת הכותרות. אם יש בגיליון אפילו תא אחד עם
 * ערך שגיאה של נוסחה (#ERROR!/#REF!/#VALUE! וכו' - קורה בקלות בשורה
 * שהודבקה עם נוסחה מקושרת מאקסל, למשל ניקוי טלפון) - getRange().getValues()
 * על הטווח כולו נכשל בשקט ונופל לגמרי, מה שגורם לדשבורד להיראות ריק לחלוטין
 * (0 לידים בכל מקום, גם לידים תקינים). כשזה קורה, קוראים שורה-שורה במקום,
 * ומדלגים רק על השורה הספציפית שבה יש שגיאה - חשוב במיוחד כאן כי הלידים
 * נטענים ידנית מאקסל (קרה בפועל אצל IBM BOB).
 */
...
/**
 * מתוקן אוטומטית: הדבקת תא מאקסל לתוך עמודת "נייד" לפעמים מביאה איתה את
 * הנוסחה המקורית (לא רק את המספר שרואים בה) - ומכיוון שתחביר אקסל שונה
 * מ-Sheets, הנוסחה נופלת ל-#ERROR!. onEdit רץ אוטומטית (Apps Script מפעיל
 * פונקציה בשם הזה לבד, בלי הגדרה) בכל עריכה/הדבקה בגיליון, ומתקן מיד תא כזה:
 * שולף את הספרות מתוך טקסט הנוסחה (הן בד"כ עדיין "שם"), וכותב אותן כטקסט
 * רגיל. אם אין ספרות לשלוף - רק מנקה לתא ריק, כדי שלא ישאר #ERROR! שמפיל
 * את קריאת הגיליון (ר' readSheetValuesResilient_). קריטי כאן במיוחד, כי
 * הלידים לאירוע הזה נטענים ידנית מאקסל.
 */
function onEdit(e) {
  try {
    if (!e || !e.range) return;
...
function sendWhatsApp(leadIds, templateId) {
  ...
  // אין עמודה נפרדת לתאריך שליחה - מתועד כשורה בהערות (מתווספת לקיים, לא
  // דורסת), בדיוק כמו בארז קדם - זו הייתה בקשה מפורשת שלה ("וואטסאפ נשלח
  // צריך להיות רק בתיעוד של ההערות"), לא ברירת מחדל אוניברסלית, אבל אין
  // סיבה לסטות ממנה כאן בלי בקשה מפורשת אחרת.
  var now = new Date();
...
/**
 * מטפלת בתשובות נכנסות מוואטסאפ (webhook של אינפוריו, מגיע דרך ה-relay
 * של המערכת הראשית - ר' README.md) - מאתרת את הליד לפי מספר טלפון (9
 * ספרות אחרונות, כדי לא להיתקע על "0" מול "+972" בהתחלה). שני סוגי תשובה
 * שונים לגמרי:
 *  - לחיצה על כפתור בתבנית: אינפוריו שולחת את טקסט הכפתור בתוך
 *    AdditionalInfo (JSON) במפתח ButtonPayload. הטקסט הזה הופך להיות
 *    הסטטוס של הליד ישירות (לא נכתב ל"תשובת איש קשר") - בלי טבלת מיפוי
 *    בכוונה, כי טקסט הכפתור משתנה מעת לעת עם עדכוני תבנית; שואבים אותו
 *    כמו שהוא ומוסיפים לרשימת הסטטוסים אוטומטית אם הוא חדש.
 *  - טקסט חופשי אמיתי שהלקוח הקליד: נכתב ל"תשובת איש קשר" + מועד הקבלה.
 *    תשובה חדשה דורסת קודמת בכוונה (לא מצטברת) - מספיק ל"תיעוד תשובת
 *    הלקוח", בלי היסטוריית שיחה מלאה (אותה החלטה כמו בארז קדם).
 */
